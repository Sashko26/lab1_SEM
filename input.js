    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      if (row === "0") {
        ...
        if (isNaN(xl) || isNaN(yl) || isNaN(xh) || isNaN(yh) ||
            xl < 1 || yl < 1 || xh < 1 || yh < 1 ||
            xl > 10 || yl > 10 || xh > 10 || yh > 10 ||
            xl > xh || yl > yh) {
          throw new Error(`Invalid country data: ${row}`);
        }
        currentList.push({ name, xl: Number(xl), yl: Number(yl), xh: Number(xh), yh: Number(yh) });
        countriesLeftInList--;
        if (countriesLeftInList === 0) {
          if (currentList.length !== expectedNumCountries) {
            throw new Error(`Expected ${expectedNumCountries} countries but found ${currentList.length}`);
          }
          countryLists.push(currentList);
          currentList = [];
        }
...
{ 
	...
	inputMatrix.forEach((row) => {
		...
		row.forEach((elem) => {
			let country = new Country( elem.name ,elem.xl, elem.yl, elem.xh, elem.yh,newZone);
