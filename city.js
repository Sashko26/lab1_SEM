...
class City {
  constructor(x, y, parent) {
    if (x < xMIN || x > xMAX || y < yMIN || y > yMAX) {
      throw new Error(
        "Incorrect city coordinates. Enter coordinates in the range from 1 to 10."
      );
    }
    this.parent = parent;
    this.account = [];
    ...
    for (let i = 0; i < this.parent.parent.info.length; i++) {
      ...
      if (this.parent.parent.info[i] !== this.parent.name) {
        ...
        this.account.push({
          balance: initBALANCE,
          country: this.parent.parent.info[i],
        });
      }
    }
    this.x = x;
  ...
  giveMoneyToANeighbor(city) {
    ...
    for (let i = 0; i < this.account.length; i++) {
      if (this.account[i].balance % DIVISOR === 0) {
        toGiveMoney.push({
          balance: this.account[i].balance / DIVISOR,
          country: this.account[i].country,
        });
      } else {
        toGiveMoney.push({
          balance: Math.floor(this.account[i].balance / DIVISOR),
          country: this.account[i].country,
        });
        oneCoin.push({
          balance: this.account[i].balance % DIVISOR,
